...
# NOTA: A otimização requer ffmpeg instalado para formatos como M4A
# Se ffmpeg não estiver disponível, defina como False
OPTIMIZE_AUDIO = False  # Se True, converte para mono 16kHz WAV antes de enviar
TARGET_SAMPLE_RATE = 16000  # 16kHz é suficiente para transcrição de voz
TARGET_CHANNELS = 1  # Mono (voz não precisa de estéreo)

# Timeout em segundos (ajuste conforme necessário)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Transcreve um arquivo de áudio usando a API do Google Gemini.")
    parser.add_argument("audio_file", help="Caminho para o arquivo de áudio a ser transcrito.")
    parser.add_argument("--api-key", required=True, help="API Key do Google Gemini.")
    parser.add_argument("--model", default="models/gemini-1.5-flash", help="Modelo a ser usado para transcrição.")
    parser.add_argument("--optimize", action="store_true", help="Otimiza o áudio antes de enviar.")
    parser.add_argument("--output-file", help="Caminho para salvar o arquivo de transcrição.")
    args = parser.parse_args()

    # Define o caminho do arquivo de áudio a partir dos argumentos
    ...
    try:
        # Verifica informações do arquivo antes de comeÃ§ar
...
*   Organize these points into a summary with clear headings for each topic, in the same language as the transcription.
...
Your entire response should be a single markdown document.'''
        ...
        # Salva a transcrição em um arquivo, se especificado
        if args.output_file:
            ...
            # Retorna JSON com sucesso e caminho do arquivo
            import json
            print(json.dumps({
                "success": True,
