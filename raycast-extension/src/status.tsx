import { ActionPanel, Action, List, showToast, Toast, getPreferenceValues, Detail } from "@raycast/api";
...
export default function SystemStatus() {
  ...
  async function loadSystemStatus() {
    try {
      ...
      const { stdout } = await execAsync(
        `cd "${preferences.projectPath}" && ${preferences.pythonPath} system_check.py --json`
      );
  ...
  return (
    <List isLoading={isLoading}>
      <List.Section title={`Status Geral: ${getStatusIcon(status.overall)} ${status.overall}`}>
        <List.Item
          title="Hardware"
          subtitle={`CPU: ${status.hardware.cpu} | RAM: ${status.hardware.memory} | GPU: ${status.hardware.gpu}`}
          accessories={[{ text: `${status.hardware.audio_devices} dispositivos de áudio` }]}
          actions={
            <ActionPanel>
              <Action.Push
