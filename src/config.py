...
class Settings(BaseSettings):
    ...
    # Directory structure
    base_dir: Path = Path(__file__).parent.parent  # Go up from src/ to project root
    storage_dir: Path = base_dir / "storage"
    logs_dir: Path = storage_dir / "logs"  # logs now inside storage/
    recordings_dir: Path = storage_dir / "recordings"
    status_dir: Path = storage_dir / "status"  # Real-time status files for Raycast
